...
`async-task-core` is not published to any registry, and this package installs from the npm registry
on target hosts, so a `file:../async-task-core` path dep would not exist at install time and would
break the install. The surviving form is:
